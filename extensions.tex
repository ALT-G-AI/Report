...
After speaking to one of the PhD students they recommended looking at the 'Toxic Comments' Kaggle competition \cite{Jigsaw2017} kernels. Upon looking  at some of the kernels it was noted that when an intermediate language was used to enhance the dataset they saw an improvement in Loss. This is a different application however as it categorises based on the sentiment whereas this challenge is based on the stylistic properties of a given author so it's not obvious if this technique would work.
  
  As there are very few sentences currently, by using the translations, we can get more data to perform analysis on. Pavel Ostyakov \cite{Ostyakov2018} has already written a nice function that performs this function. To do this \textit{TextBlob} is used to translate the sentences in to German, Spanish and French and then back from each respective language into English. 
